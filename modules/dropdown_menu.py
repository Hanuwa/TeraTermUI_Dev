...
class DropdownMenu(tkinter.Menu, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    ...
    def _add_menu_commands(self):
        ...
        label_format = "  {}  " if sys.platform.startswith("linux") else "{}"

        for value in self._values:
            formatted_label = label_format.format(value.ljust(self._min_character_width))
            self.add_command(
    ...
    def focus_next(self):
        if self.focused_index < len(self._values) - 1:
            self.focused_index += 1
            self.focus_item(self.focused_index)
    ...
    def focus_item(self, index):
        for i in range(len(self._values)):
            self.entryconfig(i, background=self._apply_appearance_mode(self._fg_color))
        self.entryconfig(index, background=self._apply_appearance_mode(self._hover_color))

    def select_focused_item(self):
        if self.focused_index != -1:
            value = self._values[self.focused_index]
            self._button_callback(value)