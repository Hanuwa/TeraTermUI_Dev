...
class CTkComboBox(CTkBaseClass):
    ...
    def __init__(self,
        ...
        self._canvas = CTkCanvas(master=self,
                                 ...
                                 height=self._apply_widget_scaling(self._desired_height), takefocus=1)
    ...
    def _create_bindings(self, sequence: Optional[str] = None):
        ...
        if sequence is None:
            ...
            self._canvas.bind("<Return>", self._clicked)
            self._canvas.bind("<space>", self._clicked)

    def _on_focus_in(self, event=None):
        if self._state == tkinter.NORMAL and len(self._values) > 0:
    ...
    def _on_leave(self, event=0):
        ...
        if str(self._canvas.focus_get()) != str(self._canvas):
            self._canvas.itemconfig("inner_parts_right",
                                    outline=self._apply_appearance_mode(self._button_color),
                                    fill=self._apply_appearance_mode(self._button_color))
            self._canvas.itemconfig("border_parts_right",
