...
class CTkFont(Font):
    ...
    def copy(self) -> "CTkFont":
        new_font = CTkFont(family=self._family, size=self._size, weight=self.cget("weight"), slant=self.cget("slant"),
                           underline=self.cget("underline"), overstrike=self.cget("overstrike"))
        new_font._size_configure_callback_list = self._size_configure_callback_list.copy()
        return new_font
