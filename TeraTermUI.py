...
# DATE - Started 1/1/23, Current Build v0.9.0 - 5/5/24
...
# and with over 10300 lines of codes, it definitely makes things harder to work with
...
class TeraTermUI(customtkinter.CTk):
    ...
    def extract_my_enrolled_classes(self, text):
        ...
        class_pattern = re.compile(
            ...
            r"([A-Z]{1,2}|TBA)\s+(\d{4}[AP]M-\d{4}[AP]M|TBA)\s*(?:\s+([\dA-Z]*?)\s+([A-Z\d]{3,4})?)?"
            r"(?:\s+([A-Z]{1,2}|TBA)\s+(\d{4}[AP]M-\d{4}[AP]M|TBA)\s*(?:\s+([\dA-Z]*?)\s+([A-Z\d]{3,4})?)?)"
            r"?(?=\s+\b[A-Z]|\s*$)",
            re.DOTALL
        )
